...
\end{enumerate}
   ...
   \item {
  	Si dos estudiantes se seleccionan aleatoriamente ¿Cual es la probabilidad de que al menos uno esté tomando alguna clase de idiomas?\\
		...
		Consideremos el evento contrario $H^c$, es decir que dados dos alumnos, ninguno de
		ellos estén estudiando idioma.\\
		Este evento es $(I \cup F \cup A)^c$ y su probabilidad se calculó en el
		primer inciso como $P((I \cup F \cup A)^c) = 0.5$.\\
		Entonces la probabilidad de que dos estudiantes no estén estudiando idiomas
		equivale a realizar el evento dos veces, lo que corresponde a la multiplcación
		de la probabilidades de los eventos, que en este caso son el mismo.\\
		$$P(H^c) = 2P((I \cup F \cup A)^c) = 2(0.5) = 0.25$$
		Entonces, la probabilidad de que al menos uno de los estudiantes esté tomando
		un idioma es \\
		$$P(H) = 1 - P(H^c) = 1 - 0.25 = 0.75$$
